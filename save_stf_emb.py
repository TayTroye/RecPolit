...
def main(config):
    ...
    sft_embedding_table = model.t5.shared.weight.detach().cpu()

    torch.save(sft_embedding_table, config["save_path"])

    print(f"SFT Embedding Table saved. Shape: {sft_embedding_table.shape}")
